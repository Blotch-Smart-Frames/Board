...
export function computeTimelineRows(lists: List[]): TimelineRow[] {
  /* sort by compareOrder, map to {id,title} */
}
export function computeTimelineItems(tasks: Task[]): {
  items: TimelineItem[];
  hiddenCount: number;
} {
  /* filter tasks with BOTH startDate+dueDate; map to {id, rowId: listId, span: {start,end} in ms via .toMillis(), task} */
}
export function computeVisibleDates(opts: {
  rangeStart;
  rangeEnd;
  scrollLeft;
  viewportWidth;
  ...
  /* verbatim port — pure date-fns math, zero changes needed */
}
```
