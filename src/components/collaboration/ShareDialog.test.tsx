...
describe("ShareDialog", () => {
  ...
  it("does not show delete button for owner", () => {
    render(<ShareDialog {...defaultProps} collaborators={[mockCollaborators[0]]} />);
  ...
  describe("Invite functionality", () => {
    ...
    it("disables input during loading", async () => {
      const user = userEvent.setup();
      const onInvite = vi.fn().mockImplementation(
  ...
  describe("Remove functionality", () => {
    ...
    it("shows error on remove failure", async () => {
      const user = userEvent.setup();
      const onRemove = vi.fn().mockRejectedValue(new Error("Cannot remove user"));
  ...
  describe("Alert dismiss", () => {
    it("dismisses error alert on close", async () => {
      ...
      const alertCloseButton = screen.getByRole("alert").querySelector('button');
      if (alertCloseButton) {
