...
@Component({
  ...
  imports: [HlmDialogImports, HlmButton, HlmInput, HlmLabel, HlmAlert, HlmAlertDescription, NgIcon, SprintDialog],
  providers: [provideIcons({ lucidePencil, lucideTrash2, lucidePlus })],
  template: `
    <hlm-dialog #dialog>
      <hlm-dialog-content *hlmDialogPortal class="sm:max-w-md">
        <hlm-dialog-header>
          <h3 hlmDialogTitle>Sprint Management</h3>
        </hlm-dialog-header>
        ...
        <div class="flex max-h-[70vh] flex-col gap-4 overflow-y-auto py-2">
          <div>
            ...
            <div class="mt-1 flex items-center gap-2">
              ...
              <button hlmBtn variant="outline" size="sm" [disabled]="savingConfig() || configUnchanged()" (click)="saveConfig()">
                {{ savingConfig() ? 'Saving...' : 'Save' }}
              </button>
            </div>
            <p class="text-muted-foreground mt-1 text-xs">Used when auto-calculating dates for new sprints</p>
          </div>
                ...
                @for (sprint of sorted(); track sprint.id) {
                  <div class="flex items-center justify-between gap-2 rounded-md border p-2">
                    ...
                    <span class="flex shrink-0 gap-1">
                      <button hlmBtn variant="ghost" size="icon" aria-label="Edit sprint" (click)="openEdit(sprint)">
...
export class SprintManagement {
  ...
  protected readonly durationDays = signal(String(DEFAULT_DURATION_DAYS));
  ...
  protected readonly configUnchanged = computed(
    () => this.durationDays() === String(this.board()?.sprintConfig?.durationDays ?? DEFAULT_DURATION_DAYS),
  );

  open(): void {
    this.durationDays.set(String(this.board()?.sprintConfig?.durationDays ?? DEFAULT_DURATION_DAYS));
  ...
  protected async remove(sprint: Sprint): Promise<void> {
    ...
    try {
      const { canDelete, taskCount } = await this.sprintService.canDeleteSprint(this.boardId(), sprint.id);
      if (!canDelete) {
