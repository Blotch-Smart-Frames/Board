...
> Last updated: 2026-07-31, Phase 8 complete, starting Phase 9 (timeline/Gantt).
...
2. **`environment.ts` is gitignored and empty.** Firebase init throws `auth/invalid-api-key` at runtime without real creds, so the app can't be *run* end-to-end here without the user's Firebase config (copy `src/environments/environment.example.ts` → `environment.ts` and fill in). Build and unit tests do NOT need real creds (Firebase SDK handles are DI tokens, overridden in tests — see below).
...
```
As of this writing: **build green, app + spec type-check green, 273 tests passing.** The tree is left in a compiling, all-green state (note: plain `tsc` does NOT check templates, only `npm run build` does — both are run before every phase is marked done).
...
src/app/
  ...
  shared/
    ui/                             # Spartan-generated primitives (hlm-*)
  ...
  features/
    ...
    board/
      ...
      task-assignees, task-dialog                       # task-dialog now also embeds label/assignee pickers
    ...
    timeline/, calendar-sync/, collaboration/   (Phases 9-11, not started)
  app.ts / app.html                 # auth gate: spinner → sign-in → <router-outlet>
...
| 9 | Timeline/Gantt (hand-rolled on CDK — hardest phase) | ⬜ not started |
| 10 | Google Calendar sync | ⬜ not started |
| 11 | Sharing (ShareDialog incl. working remove-collaborator) | ⬜ not started |
| 12 | Polish (dark-mode consolidation, version-update toast, bundle optimization, full AXE) | ⬜ not started |
...
## Remaining phases 9-12 (notes)

- **9 Timeline/Gantt (hardest):** source uses `dnd-timeline` (no Angular equiv). Hand-roll on CDK drag-drop + a pixel⇄ms time-scale service + manual resize-handle pointer logic. Preserve the business logic (optimistic span/row overrides, cross-lane detection, scroll-edge range expansion with scroll-position preservation, current-time line, sprint overlay bands). **Spike a single draggable+resizable bar first.** AppBar already has a kanban/timeline `viewMode` toggle stub. Sources: `src/components/timeline/*`, `src/hooks/{useTimelineData,useVisibleDates}.ts`.
- **10 Calendar sync:** `CalendarService` + `SyncService` already ported. Wire the sync toggle into TaskDialog (was intentionally omitted in Phase 5). **Fix from source:** toggling sync OFF must call `unlinkTaskFromCalendar` (source orphaned the event — already fixed in `SyncService`). Access token: `AuthStore` sets it on `CalendarService` at login (memory-only; lost on reload — inherent, out of scope).
- **11 Sharing:** ShareDialog (invite by email via `UserService.getUserByEmail` + `BoardService.shareBoard`; list collaborators; **remove works** via `BoardService.removeCollaborator` — source's was a no-op stub). Wire the AppBar "share" button + sidebar "Share" menu item (omitted in Phase 4 — add it now). Source: `src/components/collaboration/ShareDialog.tsx`.
- **12 Polish:** consolidate dark-mode into the theme, version-update toast (`VersionCheckService` + `sonner`; `useVersionCheck` source + `scripts/stamp-version.mjs` already stamps `BUILD_HASH`), CDK drag styling, and revisit bundle size (~1.61MB raw / ~364kB gzipped as of Phase 8 — mostly Firebase; consider lazy-loading; warning budget of 1.5MB is now exceeded by ~112kB, error budget is 2.5MB in `angular.json` and not yet at risk). Full AXE pass.

## Deliberate deviations from source (keep these)
- Not installed: `nanoid`, `zustand` (unused in source).
- Not ported: `CalendarSync.tsx` (built but never rendered in source).
- Fixed bugs: calendar unlink-on-disable; ShareDialog remove-collaborator; board reorder rollback.
- Simplified: markdown editing = textarea + preview (no WYSIWYG toolbar); completed-tasks disclosure = native `<details>`.
- Keyboard DnD: menu-based "Move" actions instead of a keyboard-drag engine (per user).
